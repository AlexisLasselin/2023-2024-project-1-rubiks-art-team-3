...
This document outlines the process of creating the fresco in two key stages:
...
These two stages will run concurrently over a three-week period, beginning on Wednesday, October 11th and concluding on the final delivery date, Thursday, October 26th.

## Introduction

While our fresco project may not be the most complex, we recognize that there are potential challenges during its construction.Therefore, we are committed to implementing strict quality assurance methods to ensure the project's success, timely completion, and long-term durability.

We encourage each group to devise their own effective method for configuring the Rubik's Cube, as they are motivated to do so.As the creators of the selected fresco, we believe we are best suited to manage its construction.

Our approach involves all groups producing Rubik's cubes in their respective rooms using their preferred methods.Each group will be responsible for assembling one-seventh of the final Fresco (referred to as "slices"), which will then be further divided into 11 equal parts (referred to as "chunks").Each chunk will have a corresponding cardboard grid on which the cubes are placed, making progress tracking and verification easier.

One member from our group will serve as the General Quality Officer, visiting each room in succession to verify progress and collect completed chunks for delivery to the library for final assembly.We expect each group to elect a quality officer responsible for their team's work.

The rest of the management of the project is left to the discretion of each group.There will be two types of teams: the managing team and the solvers.
...
<ul>
<li>Fresco Assembler: Responsible for assembling the Fresco with the chunks provided by the Checker.</li>
<li>Chunk Checker: Inspects the quality of the chunks delivered by the courier and queues them for assembly.</li>
<li>Courier: Delivers messages, restocks cubes, retrieves completed and stored chunks, and collects cubes with issues.</li>
<li>Cube Master: Ensures the structural integrity of the fresco, resets cubes with issues, and assists with other tasks.</li>
<li>Director of Operations: Makes decisions regarding strategy changes, observes and monitors processes as a whole, and decides on the reassignment of roles.Also assists with various tasks.</li>
<li>General Quality Officer: Visits each room individually to check progress, inspect stored/finished chunks, and calls the courier to retrieve them when necessary.</li>
</ul>

## Solving Team Role Descriptions

<ul>
<li>Quality Officer: Each group selects a quality officer who is accountable for quality control.</li>
<li>Rubik's Solver: The remaining team members focus on solving the Rubik's cubes.</li>
</ul>
...
Since groups are never on project time simultaneously, each group will be independant and finished chunks will be stashed in the project room it has been completed in until the management team comes to check it and collect it.The same applies for softlocked cubes, they will be stashed until our cube resetter is available to unlock them.
...
Constant checking with the fresco by visiting each rooms will allow us to react if ever a group is severely lagging behind.

Depending on their lag, several strategies can be adopted.

As we prepared our own software to solve Rubik’s cubes and check for quality we can provide those to any team if they are missing such tools.
...
* Otherwise, once they reached their objective, they double check the pixels correspondance to the software.And place the cube on the carboard chunk grid being aware that the orientation of the Rubik's cube matters.
...
* If a problem is found, the Chunk is sent back to the group it came from or to another group that is more advanced, depending on the Director of Operation's decisions.
* The Chunk Checker queues the Chunk in the assembling line, to respect the order of placement of each team as they arrived.
